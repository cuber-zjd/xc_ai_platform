...
服务器 `192.168.14.44` 的代码发布必须使用仓库根目录 `scripts/deploy_server.ps1`：本地在 `main` 精确提交并推送到内网发布仓库 `http://192.168.14.111:9055/zhangjide/ai_platform.git` 后，以 `-ExpectedCommit <commit>` 部署到原目录 `/home/xinxi/ai_platform`。不得使用 `scp` 覆盖代码、不得新建发布目录、不得绕过脚本直接在服务器拼接临时发布命令；仅后端变更使用 `-BackendOnly`。
...
- Insight 重点垂直渠道必须进入每日轻量采集池，不得只依赖到期监测对象轮转。头条、搜狐、腾讯、新浪财经默认每日覆盖全部核心业务主题；FoodDaily、食品伙伴网、咖门、小食代、食业头条、新营养、云糖、粮油市场报、新蛋白、新华网、人民网、央视等重点站点每日轮转至少两个相关主题。不同渠道低并发、同一渠道严格串行并保留冷却和失败审计；周期深挖不得在同一轮重复执行已由每日轻量池负责的渠道。
...
- Insight 默认搜索发现源当前包括“百度资讯”“博查搜索”和“豆包联网搜索”。三者都是独立渠道源：百度资讯作为低成本单对象探针，每个 active 监测对象每天执行一次，但生产调度不得在凌晨主任务中集中请求；启用 `INSIGHT_SCHEDULER_BAIDU_SPREAD_ENABLED` 后按稳定监测 ID 分配到全天时间槽，默认每 10 分钟一个槽、同一出口严格串行。百度返回安全验证、验证码域名、HTTP 403/429 等反爬信号时必须立即熔断，熔断状态写入 Redis 并跨进程、跨重启共享，记录真实失败和熔断跳过数，不得把验证码页记为成功或通过密集重试绕过验证。博查作为结构化搜索 API 补充，豆包联网搜索通过火山方舟 Responses API 的 `web_search` 工具补充发现并保留搜索动作、引用标注和模型整理后的结构化线索。博查和豆包也需要每日覆盖，但必须按客户组、竞对组、行业主题、政策主题等合并监测对象后并发执行，再将命中结果反向归属到具体监测配置，禁止按模块、按关键词、按网站笛卡尔积重复调用。豆包默认每组 2 个监测对象，单组失败可继续拆分补偿；博查/豆包分组阶段必须设置总时间预算，预算耗尽后记录并取消未完成组，继续执行重点网站适配器，不能因单一外部渠道拖延整轮调度。
...
- Insight 发布时间必须记录证据来源和核验状态。搜索结果中的相对时间只能作为待核验线索；正文补抓后只信任页面元数据、文章 URL 或明确结构化时间，正文中出现的业务事件日期不得直接作为发布时间。普通媒体发布时间未核验时只能保留候选，补抓后还必须重新执行本轮时间窗过滤。
...
- Insight 飞书日报/周报使用独立“飞书简报”模块，不写入报告中心：计划与执行记录保存于 `insight_feishu_brief_plan` / `insight_feishu_brief_run`，管理员入口为 `/insight/feishu-briefs`。文档必须由 `INSIGHT_FEISHU_BRIEF_*` 专用机器人应用创建，按领导固定版式输出总览、政策、竞对、客户、技术、原料和 7 条重点情报导读；计划的生成周期与素材周期独立配置，固定提示词需要在计划弹窗中可查看。业务规则通过 `generation_rules_json` 结构化配置关注主题、服务部门、排除项、评分阈值、栏目侧重、写作深度和来源覆盖，不得用自由 Prompt 覆盖领导固定版式、事实核验、权限或七条导读。推送支持“上午审阅组 + 下午正式接收组”：系统只生成一篇云文档，上午组获得编辑权限并先行审阅，下午到点发送修改后的同一文档，禁止重新生成覆盖上午修改；两批人员通过系统人员选择器分别维护，待发送下午批次必须持久化并可在服务重启后恢复。飞书简报按自身 `next_run_time` 到点执行，不依赖每日采集调度的单次扫描；未配置专用应用、文件夹授权或启用开关时不得发布。
- Insight 周报生成统一使用可配置多智能体链路：素材准备、策划、政策/竞对/客户/技术/原料专题研究、主笔、独立审阅、主笔修订和确定性校验；旧周报单链路不再作为运行入口。生成、审阅和正式推送时间相互独立，单期实例可临时覆盖时间、素材范围和接收人。调试运行必须使用页面当前草稿、不得推送，并在机器人写完后按当前登录用户工号转移文档所有权；固定栏目、七条导读、公司边界、事实核验、链接白名单和推送幂等不得被用户 Prompt 覆盖。
