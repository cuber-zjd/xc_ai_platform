...
- Insight 默认搜索发现源当前包括“百度资讯”“博查搜索”和“豆包联网搜索”。三者都是独立渠道源：百度资讯作为低成本单对象探针，可对每个 active 监测对象每日执行，但必须保留随机冷却、批次限流和失败记录以降低反爬风险；博查作为结构化搜索 API 补充，豆包联网搜索通过火山方舟 Responses API 的 `web_search` 工具补充发现并保留搜索动作、引用标注和模型整理后的结构化线索。博查和豆包也需要每日覆盖，但必须按客户组、竞对组、行业主题、政策主题等合并监测对象后并发执行，再将命中结果反向归属到具体监测配置，禁止按模块、按关键词、按网站笛卡尔积重复调用。豆包默认每组 2 个监测对象，单组失败可继续拆分补偿；博查/豆包分组阶段必须设置总时间预算，预算耗尽后记录并取消未完成组，继续执行重点网站适配器，不能因单一外部渠道拖延整轮调度。
