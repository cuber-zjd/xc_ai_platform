...
本文档记录 FineReport AI 报表第三步的当前对接口径：从第二步确认后的 `ReportDSL` 确定性生成 `.cpt`，上传到 MinIO staging 路径，并返回 FineReport 预览地址。当前阶段只做预览，不做正式 reportlets 发布、审批复制或覆盖。
...
```env
MINIO_ENDPOINT=192.168.14.41:9000
MINIO_ACCESS_KEY=minioadmin
MINIO_SECRET_KEY=<从服务器 minio.setting、部署密钥或新建 Access Key 获取>
MINIO_BUCKET_NAME=fanruan
...
真实 `MINIO_SECRET_KEY` 不写入仓库；如果生产环境允许，优先在 MinIO 控制台中新建专用 Access Key，并只授予 `fanruan` bucket 里 staging 路径的读写权限。
...
```text
webroot/APP/reportlets_ai_staging/{task_id}/report.cpt
webroot/APP/reportlets_ai_staging/{task_id}/report.dsl.json
webroot/APP/reportlets_ai_staging/{task_id}/query.sql
webroot/APP/reportlets_ai_staging/{task_id}/create_table.sql
webroot/APP/reportlets_ai_staging/{task_id}/generation.log
```
...
AI staging 预览第一版使用任务路径：

```text
http://192.168.14.41:1080/webroot/decision/view/report?viewlet=reportlets_ai_staging/{task_id}/report.cpt
```

如果实测 FineReport 需要从 `APP` 或 `reportlets` 根目录解析 staging，需要调整 `PreviewValidator` 中传入的 `reportlet_path`，但 MinIO 对象仍保持 `webroot/APP/reportlets_ai_staging/{task_id}/report.cpt`。
...
4. `MinIOStagingService` 上传 CPT、DSL、SQL、建表 SQL 和日志到 staging。
5. `PreviewValidator` 调用 FineReport 预览 URL，记录 `previewUrl`、HTTP 状态、`warnings` 和 `errors`。
6. 当前 `publish` 只允许标记任务状态，不得复制到正式 `reportlets`。

## 6. 验证方式

- 生成任务后，在 MinIO 控制台确认 `fanruan/webroot/APP/reportlets_ai_staging/{task_id}/report.cpt` 存在。
