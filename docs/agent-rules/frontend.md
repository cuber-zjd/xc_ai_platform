...
- Insight 飞书简报页面 `/insight/feishu-briefs` 支持日报、周报和月报计划。计划弹窗必须用结构化控件维护关注主题、服务部门、排除内容、评分阈值、栏目侧重、写作深度和来源覆盖，不得要求业务用户反复修改整段 Prompt；领导固定版式、七条重点导读、事实核验和权限规则不可由计划覆盖。月报计划必须展示生成策略和每月执行日；最近执行区以卡片展示候选稿、审校记录、参与模型和评分，避免只有最终文档链接而无法比较生成策略。
