...
25.5. 飞书月报独立执行“资料审批 -> 单模型整稿/分章节并行/多智能体研究候选 -> 事实、相关度、管理表达三审 -> 多策略择优合成 -> 终稿复审 -> 确定性格式和链接检查”。候选稿、审校记录和最终稿分别保存为飞书云文档；只有最终稿通过发布闸门后才允许卡片推送。各角色的模型从 `sys_model` 动态选择并记录实际模型和耗时，不得在业务代码中硬编码模型名称。
