...
4. 节点启用范围独立保存在 `weaver_ai_review_node_config`；手动智审要求节点总开关开启，`submit/action` 自动预审还要求节点开启自动预审。所有调用在证据查询和模型调用前统一校验。
5. 泛微后端可在确需提交后预审的节点前或节点后配置 `WeaverAiReviewAction`，在流程流转时调用 `/ai-api/v1/weaver/ai-assistant/review/precheck`；未开启自动预审时接口直接拒绝，不产生智审记录。
6. 智审服务按 `env + workflow_id + node_id + reviewer_user_id` 匹配启用规则；规则启用只读证据工具时，先根据泛微建模元数据解析并查询关联业务数据，执行确定性核验，再把表单快照、审批动作、规则快照和工具证据交给模型生成结构化预审结果。
7. 管理员可在智审配置页输入 `requestId` 发起测试审批。平台先校验请求属于当前流程，再直接读取泛微主表和全部明细表，忽略请求所处节点与节点开关，使用当前流程全部启用规则生成结果；弹窗复用审批页结果组件，记录只进入测试表，不参与正式最近记录、自动预审或流程动作。
6. 预审结果保存到 `weaver_ai_review_record`，包含风险等级、建议结论、检查项、缺失材料、关注点和建议审批意见。
7. 初版只做建议展示和审计沉淀，不直接替审批人执行同意、退回、拒绝、保存或提交；后续替审必须显式授权、限制低风险场景并保留完整审计。
