...
6. FastAPI 生命周期按 `WEAVER_AI_REVIEW_SCHEDULER_*` 配置启动一分钟扫描器，从泛微 `workflow_currentoperator` 读取已启用节点的当前审批人待办；`weaver_ai_review_scan_task` 以环境、流程、请求、节点、审批人和待办记录做幂等，同一待办只审一次，退回后新待办可再次预审。
