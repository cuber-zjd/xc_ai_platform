...
25.3. 正式每日调度采用分层策略：凌晨主任务每日发现层运行博查、豆包分组查询和重点网站适配器；百度资讯从凌晨主任务拆出，按监测配置 ID 稳定映射到全天时间槽，默认 144 个十分钟槽覆盖 24 小时，每个启用监测对象每天恰好执行一次。百度分片使用独立后台循环、独立 PostgreSQL advisory lock 和 `baidu_spread_tick` 任务日志；服务重启只等待下一个时间槽，不集中补打历史槽。搜索客户端识别验证码、安全验证域名和 403/429 后将熔断状态写入 Redis，熔断期间跳过百度但不阻塞其他渠道。信号深挖层优先处理当日新增信号，再由到期对象补位，执行按日期轮转的独立网站适配器。`INSIGHT_SCHEDULER_BATCH_LIMIT` 只限制信号深挖，不能截断博查、豆包和重点网站每日发现。博查、豆包等聚合渠道的原始搜索调用必须受 `channel_timeout_seconds` 外层超时保护，超时只结束当前批次并记录渠道错误，不得让整轮每日发现、补采汇总或后续日报永久等待；已提交的其他渠道入库结果必须保留。
...
25.5. 飞书简报支持分批发布：计划到点只生成一篇云文档，上午审阅组先获得编辑权限并收到审阅卡片，下午正式接收组收到经过修改的同一文档链接；下午阶段不得重新运行选材、写作或建文档。周报和月报在模型选材前必须复用同一套产业公司确定性门禁：健源围绕玉米深加工和糖类，御馨围绕大豆和植物蛋白；只在正文深处偶然提到核心词、或仅由 AI 评审生成“可能相关”启示的材料不能通过门禁。饮料茶饮通用主题默认归健源，御馨只接收明确涉及大豆、植物蛋白、豆乳或蛋白饮料配方的事件。
...
- 报告归并方式、表述口径、链接位置和导读写法维护在每个计划独立的 `prompt_config_json.editorial_rules`，可编辑并可恢复系统默认；当前必需栏目和七条导读数量仍由 Markdown 模板与确定性校验共同约束。
- 权限过滤、事实与来源核验、链接合法性和推送幂等属于不可关闭的安全护栏，用户 Prompt 与报告生成规则均不得覆盖。
- 周报配置页按“计划设置、素材与生成、高级编排、调试与记录”收口；常用时间与接收人不和 Agent Prompt、Markdown 模板平铺在同一层。
- 旧 `prompt_override` 已停止业务读写，历史内容先一次性并入旧主笔配置，再随旧整段写作规则迁入 `prompt_config_json.editorial_rules`；后续主笔角色要求和报告生成规则分别维护，避免安全护栏、编辑规范和智能体职责混在同一字段。
...
1. 流程审批页 ecode 先调用 `/weaver/ai-assistant/review/node-status`，只有 `env + workflow_id + node_id` 已启用且允许显示入口时才挂载图标并打开 `/weaver/assistant/review` iframe；未配置默认关闭。
2. 智审侧栏支持从左边缘拖拽调整宽度并记住浏览器本地偏好；明细对比默认异常优先，可切换对账顺序或仅看异常。
3. 发票商品与对账物料核验时，以对账单采购物料描述为主体，将发票商品名称和规格型号合并后做综合名称匹配；规格不单独判定，单位暂只展示。数量、金额、税率明确不一致时对具体单元格标红。
4. 商品名称匹配采用“精确归一化 + AI 语义消歧”混合链路：完全一致项由程序确认，非完全一致项批量交给泛微专用强推理模型判断同一商品关系；后端限制候选编号、最低置信度和一对一约束，并保留匹配方式与理由供审批人查看。
5. 节点启用范围独立保存在 `weaver_ai_review_node_config`；手动智审要求节点总开关开启，定时扫描与 `submit/action` 自动预审还要求节点开启自动预审。所有调用在证据查询和模型调用前统一校验。
6. FastAPI 生命周期按 `WEAVER_AI_REVIEW_SCHEDULER_*` 配置启动一分钟扫描器，从泛微 `workflow_currentoperator` 倒序读取已启用节点的当前审批人待办；扫描器扩大候选窗口并跳过已审项，直到领取满本轮真实执行额度，旧待办不得阻塞新待办。`weaver_ai_review_scan_task` 以环境、流程、请求、节点、审批人和待办记录做幂等，同一待办只审一次，退回后新待办可再次预审。
7. 泛微后端可在确需提交后立即预审的节点配置 `WeaverAiReviewAction` 调用 `/ai-api/v1/weaver/ai-assistant/review/precheck`；常规场景不需要逐节点挂 Action。
8. 智审服务按 `env + workflow_id + node_id + reviewer_user_id` 匹配启用规则；规则启用只读证据工具时，先根据泛微建模元数据解析并查询关联业务数据，执行确定性核验，再把表单快照、审批动作、规则快照和工具证据交给模型生成结构化预审结果。
9. 管理员可在智审配置页输入 `requestId` 发起测试审批。平台先校验请求属于当前流程，再直接读取泛微主表和全部明细表，忽略请求所处节点与节点开关，使用当前流程全部启用规则生成结果；弹窗复用审批页结果组件，记录只进入测试表，不参与正式最近记录、自动预审或流程动作。
10. 预审结果保存到 `weaver_ai_review_record`；审批页按当前审批人读取最新结果。定时预审完成后，ecode 在悬浮入口附近展示一次风险摘要提醒，点击进入完整详情。
11. 初版只做建议展示和审计沉淀，不直接替审批人执行同意、退回、拒绝、保存或提交；后续替审必须显式授权、限制低风险场景并保留完整审计。
