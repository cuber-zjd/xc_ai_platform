...
25.5. 飞书月报独立执行“资料审批 -> 单模型整稿/分章节并行/多智能体研究候选 -> 事实、相关度、管理表达三审 -> 多策略择优合成 -> 终稿复审 -> 确定性格式和链接检查”。候选稿、审校记录和最终稿分别保存为飞书云文档；只有最终稿通过发布闸门后才允许卡片推送。月报链接必须挂在自然的“主体+动作”事件短语上，禁止使用“原文、来源、详情、点击查看”等无语义链接文字；关键市场信息解读的每条事件都必须保留至少一个可回溯链接。健源终稿必须围绕大豆与植物蛋白，御馨终稿必须围绕玉米深加工与糖类业务，跨产业公司主线由确定性门禁拦截。各角色的模型从 `sys_model` 动态选择并记录实际模型和耗时，不得在业务代码中硬编码模型名称。
