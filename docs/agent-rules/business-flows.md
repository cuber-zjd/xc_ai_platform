...
4. 泛微后端可在节点前或节点后配置 `WeaverAiReviewAction`，在流程流转时调用 `/ai-api/v1/weaver/ai-assistant/review/precheck` 自动生成智审记录。
5. 智审服务按 `env + workflow_id + node_id + reviewer_user_id` 匹配启用规则；规则启用只读证据工具时，先根据泛微建模元数据解析并查询关联业务数据，执行确定性核验，再把表单快照、审批动作、规则快照和工具证据交给模型生成结构化预审结果。
