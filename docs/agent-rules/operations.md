...
- 博查与豆包的网络搜索阶段和入库评审阶段分别限流。豆包分组搜索超时后按 `INSIGHT_SCHEDULER_GROUPED_AI_RETRY_BATCH_SIZE` 拆小组重试，成功子组继续入库；入库并发由 `INSIGHT_SCHEDULER_GROUPED_INGEST_CONCURRENCY` 控制，避免正文补抓和 AI 评审长时间占满数据库连接池。相同 URL 已有有效正文时直接复用，PDF 等当前不支持的正文类型快速保留为候选并记录跳过原因。
