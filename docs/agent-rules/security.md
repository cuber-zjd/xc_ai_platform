...
- 独立飞书简报必须使用专用应用凭证，以机器人身份创建云文档；应用密钥、文件夹 Token 和默认接收人仅允许来自环境变量。简报计划接口仅管理员可访问，发布内容只允许取正式情报，并记录文档、推送结果和失败原因。启用飞书文档密级后不得通过降低密级或开放链接绕过控制：上午审阅组必须逐人授予编辑权限，下午正式接收组必须逐人授予阅读权限，推送前再次复核授权，授权失败者不发送不可访问的链接。应用应开通“获取文档密级标签名称”字段权限，执行记录保存实际密级或 `unknown`，不得把无法读取密级误记为无密级。
