...
- HR 同步使用的 SQL Server 地址、账号和密码必须来自 `HR_SYNC_MSSQL_*` 环境变量，不得写入代码、日志或同步结果；人员离职状态必须以 HR 全量人员视图返回值为准。
...
- 评审前正文补抓的直接 HTTP 降级只允许公开 `http/https` 地址，必须逐次校验重定向目标，阻断 localhost、链路本地、未指定地址和 RFC1918/IPv6 ULA 内网，限制重定向次数、响应体大小和超时；不得携带平台 Cookie、用户登录态或内部鉴权头访问外部页面。
...
- 飞书集成的 `FEISHU_APP_SECRET`、多维表格 app token、云文档文件夹 token、消息接收人 ID 必须来自环境变量或受控配置，不得硬编码；同步情报到飞书多维表格、生成飞书云文档或推送消息前必须先在后端按当前同步策略做权限过滤，避免把仅部分用户可见的情报同步到全员可见表格或文档。
- 飞书多维表格同步以正式情报 ID 作为幂等键，服务端先查询当前用户可见情报再新增或更新记录；字段创建和单选/多选选项扩展只能通过受控字段定义执行，不得允许前端提交任意飞书字段类型或未经校验的元数据修改。
- 独立飞书简报必须使用专用应用凭证，以机器人身份创建云文档；应用密钥、文件夹 Token 和默认接收人仅允许来自环境变量。简报计划接口仅管理员可访问，发布内容只允许取正式情报，并记录文档、推送结果和失败原因。配置的文件夹 Token 是机器人目录操作的根边界，自动归档只能在该根目录下按公司、年份和报告类型查找或新建子目录，不得搜索、移动或删除根目录外的文件。启用飞书文档密级后不得通过降低密级或开放链接绕过控制：上午审阅组必须逐人授予编辑权限，下午正式接收组必须逐人授予阅读权限，推送前再次复核授权，授权失败者不发送不可访问的链接。应用应开通“获取文档密级标签名称”字段权限，执行记录保存实际密级或 `unknown`，不得把无法读取密级误记为无密级。
...
- 测试审批接口同样要求 `ai-sign`，只允许读取 `WEAVER_DB_CONFIGS` 白名单环境；输入的 `requestId` 必须属于当前配置页的 `workflowId`，数据库表名和关联字段必须通过标识符白名单校验。测试模式忽略节点状态但永不触发审批动作，结果写入独立测试表。
