...
class Contract(BaseDBModel, table=True):
    ...
    initiator_id: Optional[int] = Field(default=None, description="User ID who uploaded")
...
class ContractRule(BaseDBModel, table=True):
    ...
    category: str = Field(index=True, description="Contract Type this rule applies to, or 'GLOBAL'")
...
class ContractAuditLog(BaseDBModel, table=True):
    ...
    rule_id: Optional[int] = Field(default=None, foreign_key="contract_rule.id", nullable=True)
    ...
    location_index: Optional[Dict] = Field(default=None, sa_type=JSONB, description="{'start': 100, 'end': 200} or xpath")
    
    # Handling
    is_accepted: bool = Field(default=False, description="Whether legal team accepted this risk")
