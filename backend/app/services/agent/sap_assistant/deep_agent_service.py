...
- 每拿到一个工具结果，都要重新判断：证据是否足够、下一步是否应该查源码、追函数、查 DDIC、查只读数据或总结。
- 对需要看源码再查数据的问题，优先按“定位事务码/对象 -> 读取源码包 -> 追真实 CALL FUNCTION/PERFORM -> 查 DDIC 或只读样例数据补证 -> 总结”的循环推进。
...
class SapDeepAgentService:
    ...
    async def run(
        ...
        intent_item = self._timeline("intent", "success", "开始分析", "正在结合会话上下文和可用 SAP 工具规划排查路径。")
    ...
    def _build_tools(
        ...
        async def safe_table_read(
            ...
            """通过 SAP 侧只读 RFC 小批量读取数据。必须显式给 1-8 个字段、至少一个高选择性 ranges 条件，max_rows 建议 1-5，避免 RFC_READ_TABLE 缓冲超出。"""
    ...
    def _system_prompt(self, system: SapSystemConfig | None) -> str:
        ...
        return (
            ...
            "4. 源码调查先用 program_source/function_source 读取对象；系统会完整拉取源码并写入审计，但默认只返回与用户问题相关的源码包，包括关键 SELECT/WHERE、赋值、PERFORM、CALL FUNCTION 和相邻上下文。\n"
            "5. 如果源码包足以定位表、字段或过滤条件，下一步优先查 DDIC 或 safe_table_read 对照数据；safe_table_read 必须少字段、少行、强条件：fields 显式给 1-8 个必要字段，max_rows 默认 5，必须带主键/凭证号/公司/日期等高选择性 ranges，不要空 fields 或无条件读表。\n"
            "6. 可以沿真实 CALL FUNCTION 递归读取被调用函数的源码包，但不要重复读取完全相同对象；如果某个方向无关，果断换函数、查 DDIC、只读数据或总结不确定性。\n"
            "7. 字段取值、金额计算、字段血缘问题必须有可执行代码证据，例如 SELECT、LOOP、READ TABLE、CALL FUNCTION、PERFORM、赋值或计算语句；注释、标题、DDIC 只能作为弱证据。\n"
            "8. 每轮观察后维护证据链：强证据、弱证据、缺口和不确定性。证据足够时必须调用 finish_investigation 进入后端流式总结，不要继续泛搜。\n"
            "【预算与递归策略】\n"
            f"1. 本轮最多调用 {self.MAX_TOOL_CALLS} 次业务工具。预算紧张、方向发散或需要拆分子任务时，先调用 compact_investigation_state 查看证据账本和剩余预算。\n"
            "2. 如果核心证据仍不足，只选择一个最高价值的后续动作；如果剩余步骤只是知识库、样例数据等补强，可以跳过并在最终总结中标注不确定性。\n"
    ...
    def _source_subagent_prompt(self) -> str:
        return (
            ...
            "优先顺序：事务码对应程序 -> program_source/function_source 获取源码包 -> 定位赋值/计算/过滤条件 -> 查表数据对照 -> 必要时 source_full_text。"
    ...
    async def _compose_budget_answer(
        ...
        try:
            ...
            if not evidence:
                lines.append("- 暂无可用证据。")
            lines.extend(["", "如需确定字段取值、金额计算或血缘关系，需要继续取得可执行源码、DDIC、日志或只读样例数据。"])
            answer = "\n".join(lines)
    ...
    def _friendly_skip_detail(self, detail: str) -> str:
        if "safe_table_read 已拦截" in detail:
            if "fields" in detail or "空 fields" in detail:
                return "本次读表请求缺少明确字段，已自动收窄，避免读取过宽数据。"
            if "ranges" in detail or "无条件" in detail:
                return "本次读表请求缺少足够精确的过滤条件，已暂停执行，避免无条件读取业务表。"
        if "已经执行过" in detail:
    ...
    def _tool_pending_detail(self, tool_name: str, params: dict[str, Any]) -> str:
        ...
        if tool_name == "safe_table_read":
            ...
            return (
                f"正在通过受控只读 RFC 读取 {table_name}：字段 {len(fields)} 个、条件 {len(ranges)} 个、最多 {max_rows} 行。"
                "该工具会自动拒绝空字段或无条件读表，以降低 RFC_READ_TABLE 缓冲超出风险。"
            )
    ...
    def _user_friendly_tool_detail(self, tool_name: str, params: dict[str, Any], status: str, summary: str, data: Any) -> str:
        ...
        if tool_name == "ddic_meta":
            ...
            return f"已读取 {object_name} 的字段和结构信息，用于校验后续读表条件。"
        if tool_name == "safe_table_read":
            table_name = params.get("table_name") or "目标表"
            return f"已按收窄条件读取 {table_name} 的少量数据，用于和源码逻辑对照。"
