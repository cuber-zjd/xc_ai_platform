...
def crawl_cndrink(keyword):
    # 采用 Foodaily 平台代署名代理方案来绕过主站 Cloudflare 的 403 封锁
    search_kw = f"中国饮品快报 {keyword}"
    print(f"开始利用 Foodaily 平台间接爬取中国饮品快报 - 检索词: '{search_kw}'")
    articles = []
    ...
    with sync_playwright() as p:
        ...
        for card in cards:
            try:
                ...
                parts = ut_text.split("\n")
                author = parts[0].strip() if len(parts) > 0 and parts[0].strip() else "中国饮品快报"
                date_raw = parts[1].strip() if len(parts) > 1 else ""
                created_at = clean_time(date_raw)
                
                if "饮品快报" in author or "饮品快报" in title or "中国饮品快报" in content:
                    articles.append({
                        "title": title,
                        "author": "中国饮品快报",
