...
def crawl_xiaoshidai(keyword):
    # 采用今日头条资讯代理方案，精准检索“小食代”官方公众号发表的新茶饮特稿
    search_kw = f"小食代 {keyword}"
    print(f"开始利用头条平台间接爬取小食代 - 检索词: '{search_kw}'")
    articles = []
    ...
    with sync_playwright() as p:
        ...
        for card in cards:
            try:
                ...
                author_raw = parts[0].strip() if len(parts) > 0 and parts[0].strip() else "小食代"
                
                # 清洗作者名称，剥离时间后缀
                author = re.split(r'(昨天|前天|\d+天前|\d+小时前|\d+分钟前|\d+月\d+日|\d{4}[-.年]\d{1,2}[-.月]\d{1,2})', author_raw)[0].strip()
                
                # 核心校验：作者必须包含小食代
                if "小食代" not in author:
                    continue
                    
                time_raw = ""
                for p_val in reversed(parts):
                    p_val = p_val.strip()
                    if "评论" not in p_val and p_val != author_raw:
                        time_raw = p_val
                        break
