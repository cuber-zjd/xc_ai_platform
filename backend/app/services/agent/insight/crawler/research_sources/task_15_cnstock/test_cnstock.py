...
def crawl_cnstock(keyword):
    # 采用今日头条资讯代理方案，精确检索“中国证券网”及“上海证券报”关于新茶饮的权威报道
    search_kw = f"中国证券网 {keyword}"
    print(f"开始利用头条平台间接爬取中国证券网 - 检索词: '{search_kw}'")
    articles = []
    ...
    with sync_playwright() as p:
        ...
        print(f"  [中国证券网] 找到候选卡片数: {len(cards)}")
        ...
        for card in cards:
            try:
                ...
                author = parts[0].strip() if len(parts) > 0 and parts[0].strip() else "中国证券网"
                
                # 核心校验：作者必须为 中国证券网 或 上海证券报，以排除外部杂音
                if "中国证券网" not in author and "上海证券报" not in author:
                    continue
